...
public class ExternalChainingHashMap<K, V> {
    ...
    public ExternalChainingHashMap(int initialCapacity) {
        this.table = (ExternalChainingMapEntry<K, V>[]) new ExternalChainingMapEntry[initialCapacity];
        size = 0;
    ...
    public V put(K key, V value) {
        ...
        if (key == null) {
            throw new IllegalArgumentException("The key to be added in the array can't be null");
        }

        if (value == null) {
            throw new IllegalArgumentException("The value to be added in the array can't be null");
        }
        ...
        ExternalChainingMapEntry<K, V> temp = table[hash];
    ...
    public V remove(K key) {
        if (key == null) {
            throw new IllegalArgumentException("Key to be removed from the table array can't be null");
        }
        int hash = Math.abs(key.hashCode() % table.length);
        ExternalChainingMapEntry<K, V> curr = table[hash];

        if (curr == null) {
            throw new NoSuchElementException("The table array is null, so no key found");
        }
        ...
        throw new NoSuchElementException("Key whose value is to be removed not found in the array");
    }
    ...
    public V get(K key) {
        if (key == null) {
            throw new IllegalArgumentException("The given key is null. Can't have null key.");
        }
        int hash = Math.abs(key.hashCode() % table.length);
        ExternalChainingMapEntry<K, V> curr = table[hash];

        if (curr == null) {
            throw new NoSuchElementException("Key can't be found in map since the array is null");
        }
    ...
    public void resizeBackingTable(int length) {
        ...
        if (length < size) {
            throw new IllegalArgumentException("Length of the array is less than the number of items in the array");
        }

        ExternalChainingMapEntry<K, V>[] tempArray;
        tempArray = (ExternalChainingMapEntry<K, V>[]) new ExternalChainingMapEntry[length];
        ...
        for (int i = 0; i < table.length; i++) {
            ...
            while (curr != null) {
                ...
                ExternalChainingMapEntry<K, V> add;
                add = new ExternalChainingMapEntry<>(curr.getKey(), curr.getValue());
    ...
    public void clear() {
        ...
        temp = (ExternalChainingMapEntry<K, V>[]) new ExternalChainingMapEntry[INITIAL_CAPACITY];
        table = temp;
        size = 0;
    }
